...
| **HashKey Chain** | EVM RPC (OP Stack) | Merkle 审计日志锚定 + 部署环境 (Development)
...
│   │   │   │   ├── svm.rs        # Solana RPC
│   │   │   │   ├── btc.rs        # Bitcoin RPC
│   │   │   │   ├── stellar.rs    # Stellar/Soroban RPC (v1.5/Development 专项)
│   │   │   │   ├── hashkey.rs    # HashKey Chain RPC (EVM, Development)
...
**Development 加分**：Telegram Mini App 是最直观的 Demo 展示方式，用户无需离开 Telegram 即可完成完整 Agent 钱包操作流程。
...
HashKey Chain + BNB Chain + X Layer + Telegram Mini App 可同时作为 Development 多链+多端演示环境 — 在 BSC 上跑通支付/DeFi (PancakeSwap)，在 X Layer 上跑通 x402 Agentic Payment (OKX Onchain OS)，在 HSK 上跑通 Merkle 审计锚定 (AuditAnchor 合约)，展示完整的"策略保护 + 合规可证明" Agent 钱包平台。
...
**Development 匹配** (Build X, 截止 4 月 15 日):
...
---
### 5.8 Stellar 专项支持（Development 方向）
...
Development 最小可打 Demo 路径：OWS Vault → Stellar 签名 → 链上交易 → Policy Engine 审计，展示"Agent 在策略保护下执行链上操作"的完整闭环。
