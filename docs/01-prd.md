...
```bash
# 身份
gradience auth login          # Passkey 认证
...
- [ ] CLI 可以安装并运行
- [ ] 用户可以通过 Passkey 注册/登录
- [ ] 用户可以创建 Agent 钱包并看到多链地址
- [ ] 用户可以设置基础 Policy (限额)
- [ ] MCP Agent 可以通过 OWS 接口签名和发送交易
- [ ] Policy 在实际签名前生效
- [ ] 支持 OWS 全部链类型
- [ ] 代码开源 (MIT)，有完整 README
...
- [ ] 动态策略至少支持 Reputation 与市场风险两个信号
- [ ] 交易意图解析与匹配测试通过率 ≥ 95%
- [ ] 审计日志包含 intent 与 dynamic factors
