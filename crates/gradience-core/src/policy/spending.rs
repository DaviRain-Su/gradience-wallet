...
fn resolve_reset_at(period: &str, now: chrono::DateTime<chrono::Utc>) -> chrono::DateTime<chrono::Utc> {
    match period {
            ...
            match rule {
                ...
                Rule::SharedBudget { max, token, period } => {
                    ...
                    if spent > limit_wei {
                        return Ok(EvalResult {
                            decision: Decision::Deny,
                            reasons: vec![format!("shared budget exceeded for workspace {}", ws_id)],
                            matched_intent: None,
                            dynamic_adjustments: vec![],
                        });
                    }
