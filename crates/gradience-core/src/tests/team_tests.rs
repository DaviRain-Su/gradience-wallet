...
use crate::team::workspace::{WorkspaceRole, WorkspaceService};
...
async fn test_create_workspace_success() {
    ...
    let id = svc
        .create_workspace(&db, "development-team", "user-1")
        .await
        .unwrap();
    assert!(!id.is_empty());
}
...
async fn test_add_member() {
    ...
    svc.add_member(&db, &ws_id, "user-2", WorkspaceRole::Member)
        .await
        .unwrap();
    let members = gradience_db::queries::list_workspace_members(&db, &ws_id)
        .await
        .unwrap();
...
fn test_role_from_str() {
    ...
    assert_eq!(
        "member".parse::<WorkspaceRole>().unwrap(),
        WorkspaceRole::Member
    );
    assert_eq!(
        "viewer".parse::<WorkspaceRole>().unwrap(),
