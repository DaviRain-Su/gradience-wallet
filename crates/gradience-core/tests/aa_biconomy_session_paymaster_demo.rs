...
use gradience_core::aa::paymaster::{PaymasterClient, apply_paymaster};
...
#[tokio::test]
async fn demo_biconomy_session_paymaster() {
    ...
    if project_id.is_empty() || client_key.is_empty() {
        println!("SKIP: set PARTICLE_PROJECT_ID and PARTICLE_CLIENT_KEY to run this paymaster demo.");
        println!("      Get your keys at https://dashboard.particle.network");
        return;
    }
    ...
    // Deterministic session signer so Phase 1 and Phase 2 remain consistent across runs.
    ...
    // Phase 1: enableModule + setMerkleRoot (owner-signed, no paymaster)
    sol! {
    ...
    // Phase 2: Session-signed Execute via Paymaster
    ...
    // Dummy signature for paymaster sponsorship request
    op_exec.signature = Bytes::new();

    let paymaster = PaymasterClient::new(&project_id, &client_key);
    let sponsor = paymaster
        .sponsor_user_op(chain_id, &entry_point.to_string(), &op_exec)
        .await
        .expect("paymaster sponsorship failed");

    println!("PaymasterAndData: {}", sponsor.paymaster_and_data);

    // Inject paymaster data and re-sign with session key
    apply_paymaster(&mut op_exec, &sponsor.paymaster_and_data).unwrap();
    ...
    println!("Phase 2 with Paymaster tx hash: {:?}", receipt_p2.transaction_hash);
}
