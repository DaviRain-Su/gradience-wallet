...
pub async fn invite(
    ...
    svc.add_member(&ctx.db, &workspace_id, &user_id, role)
        .await?;
    println!(
        "Invited {} to workspace {} as {:?}",
        user_email, workspace_id, role
    );
    Ok(())
}
...
pub async fn budget_set(
    ...
    svc.allocate_workspace_budget(&ctx.db, &workspace_id, wei, &token, &chain_id, &period)
        .await?;
    println!(
...
pub async fn budget_status(
    ...
    let svc = gradience_core::team::shared_budget::SharedBudgetService::new();
    let remaining = svc
        .get_remaining_budget(&ctx.db, &workspace_id, &token, &chain_id, &period)
        .await?;
