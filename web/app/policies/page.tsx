...
export default function PoliciesPage() {
  ...
  async function fetchWallets() {
    try {
      ...
      // Load policies for each wallet
      data.forEach((w: Wallet) => fetchPolicies(w.id));
    } catch (e: unknown) {
      setMsg(`Failed to load wallets: ${e instanceof Error ? e.message : String(e)}`);
    }
  }

  async function fetchPolicies(walletId: string) {
    try {
      const res = await apiGet(`/api/wallets/${walletId}/policies`);
      const data = await res.json();
      setPolicies((prev) => ({ ...prev, [walletId]: data }));
    } catch {
      // ignore
    }
  }
  ...
  async function handleCreate() {
    if (!selectedWallet || !policyName.trim()) return;
    setLoading(true);
    try {
      ...
      setPolicyName("");
      await fetchPolicies(selectedWallet);
    } catch (e: unknown) {
